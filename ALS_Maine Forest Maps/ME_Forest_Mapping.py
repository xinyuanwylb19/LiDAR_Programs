...
if data_prepare:
    ...
    # One map per year and product, on the Maine grid. Cover marks unprocessed ground as no data.
    profile = {"driver": "GTiff", "height": height, "width": width, "count": 1, "dtype": "uint8",
               ...
               "BIGTIFF": "IF_SAFER"}
    products = [("Change", Lcms_dir, Change_zip, Change_tif, Change_names, Loss_classes, None),
                ("Cover", Cover_dir, Cover_zip, Cover_tif, Cover_names, sorted(Cover_names), Skip_class)]
    for tag, folder, archive, name, names, report, skip in products:
        for year in Years:
            out_path = os.path.join(folder, "%d_%s_Maine.tif" % (year, tag))
            if os.path.exists(out_path):
                print("%s %d already done" % (tag, year))
                continue
            start = time.time()
            source = "/vsizip//vsicurl/" + (archive % (Version, year)) + "/" + (name % (Version, year))
            ...
            try:
                ...
                if os.path.exists(part_path):
                    os.remove(part_path)
                print("%s %d failed, run the script again to retry it: %s" % (tag, year, trouble))
                continue
            total = counts.sum() - counts[Lcms_nodata]
            print("%s %d written in %.0f s, %s" % (tag, year, time.time() - start, out_path))
            for code in sorted(report, key=lambda c: -counts[c]):
                if counts[code] > 0:
                    print("    %-28s %11d cells, %6.3f%% of the state"
                          % (names[code], counts[code], 100.0 * counts[code] / max(total, 1)))
...
if reconstruction:
    ...
    profile = {"driver": "GTiff", "height": height, "width": width, "count": 1, "dtype": "int16",
               "crs": Maine_crs, "nodata": Annual_nodata, "transform": sample.transform,
...
if delineation:
    ...
    height, width = sample.height, sample.width
    profile = {"driver": "GTiff", "height": height, "width": width, "count": 1, "dtype": "uint8",
               "crs": Maine_crs, "nodata": 0, "transform": sample.transform,
               "compress": "deflate", "tiled": True, "blockxsize": 256, "blockysize": 256,
               "BIGTIFF": "IF_SAFER"}
