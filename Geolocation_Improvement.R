...
#-------------------------------------------------------------------------------
# Function: plot trees in local coordinates
plot_trees <- function(data, circle_data, title) {
  ggplot(data, aes(x = x, y = y, size = DBH, fill = Species)) +
    ...
    scale_fill_manual(values = scales::hue_pal()(length(unique(data$Species))),
                      name = "Species",
...
# Plot center in absolute coordinates
e0 <- plot_data[[east]][1]
...
# Map all trees in local coordinates
plot_trees(tree_data, circle_data,
           paste("All Trees in Plot", plot_id))

#-------------------------------------------------------------------------------
# Inventory data filtering
# Filter 1: big trees by DBH threshold
...
cat("Number of big trees after DBH filter:", nrow(big_trees), "\n")

# Map big trees in local coordinates
plot_trees(big_trees, circle_data,
           paste("Big Trees (DBH \u2265", round(db_thresh, 1), "cm) in Plot", plot_id))

# Filter 2: remove trees within tree_dist m of each other
inv_abs <- big_trees %>%
  transmute(x = x, y = y, X_abs = Xinv, Y_abs = Yinv, DBH = DBH, Species = Species)
...
# Map thinned trees in local coordinates
plot_trees(filter_trees, circle_data,
           paste("Thinned Trees (\u2265", tree_dist, "m apart) in Plot", plot_id))

#-------------------------------------------------------------------------------
# Read & merge LiDAR tiles.
...
#-------------------------------------------------------------------------------
# Tree segmentation and derive centroids using LAS data
...
# Overlay: filtered inventory trees on LiDAR canopy map (before correction)
circle_orig_sf <- st_sf(geometry = st_buffer(
  st_sfc(st_point(c(e0, n0)), crs = st_crs(crowns_sf)), dist = plot_radius))

ggplot() +
  geom_sf(data = crowns_sf, fill = NA, color = "green") +
  geom_sf(data = circle_orig_sf, fill = NA, color = "black", linetype = "solid", linewidth = 1) +
  ...
  coord_sf(xlim = c(x_min, x_max), ylim = c(y_min, y_max), datum = NA, expand = FALSE) +
  theme_minimal() +
  labs(title = paste("Plot", plot_id, "– Crowns, Centroids & Filtered Inventory Trees"),
       x = "Easting (m)", y = "Northing (m)")

#-------------------------------------------------------------------------------
# Improve plot location
...
# Find the nearest centroid for each field point
